...
# === HELPERS ===
def send_with_sl_fallback(req, is_buy, price):
    r = mt5.order_send(req)
    if r is None or r.retcode != mt5.TRADE_RETCODE_DONE:
        logger.error(f"Order failed (retcode={getattr(r, 'retcode', None)}), retrying with fallback SL")
        # apply minimal SL *2
        fallback = MIN_SL * 4
        req['sl'] = price - fallback if is_buy else price + fallback
        r2 = mt5.order_send(req)
        if r2 and r2.retcode == mt5.TRADE_RETCODE_DONE:
            logger.info(f"Fallback order succeeded: SL set to {req['sl']:.5f}")
            return r2
        logger.error(f"Fallback order failed (retcode={getattr(r2, 'retcode', None)})")
    return r
...
def execute_phase_trade(is_buy, phase, df1):
    ...
    tick=mt5.symbol_info_tick(SYMBOL); price=tick.ask if is_buy else tick.bid
    for _ in range(cnt):
        req={"action":mt5.TRADE_ACTION_DEAL,"symbol":SYMBOL,
             "volume":LOT_SIZE,"type":mt5.ORDER_TYPE_BUY if is_buy else mt5.ORDER_TYPE_SELL,
             "price":price,"sl":price,"deviation":DEVIATION,
             "magic":MAGIC,"comment":f"Entry phase{phase}",
             "type_time":mt5.ORDER_TIME_GTC,"type_filling":mt5.ORDER_FILLING_FOK}
        r=send_with_sl_fallback(req,is_buy,price)
...
def close_position(pos):
    ...
    req={"action":mt5.TRADE_ACTION_DEAL,"symbol":SYMBOL,"volume":pos.volume,
         "type":ct,"position":pos.ticket,"price":price,
         "deviation":DEVIATION,"magic":MAGIC,"comment":"Auto close",
         "type_time":mt5.ORDER_TIME_GTC,"type_filling":mt5.ORDER_FILLING_FOK}
    r=mt5.order_send(req)
    if r and r.retcode==mt5.TRADE_RETCODE_DONE:
...
def modify_sl(pos,new_sl):
    ...
    r=send_with_sl_fallback(req,pos.type==mt5.ORDER_TYPE_BUY,new_sl)
    if r and r.retcode==mt5.TRADE_RETCODE_DONE:
